...
{
    $petugas = User::paginate(3); // Pagination for petugas
    $pengaduans = Pengaduan::with('masyarakat', 'kategori')->latest()->paginate(3); // Pagination for pengaduans
    return view('pages.admin.laporanmasuk.index', compact('petugas', 'pengaduans'));
}
     ...
     */
     ...
     public function create()
     {
        ...
        $masyarakats = User::all();
         $kategoris = KategoriPengaduan::all(); // Pastikan Kategori memiliki data
    ...
    public function edit(Pengaduan $pengaduan , $id)
    {
        ...
        return view('pages.admin.laporanmasuk.index', compact('pengaduan', 'masyarakats', 'kategoris'));
    }
...
public function tanggapan()
{
    ...
    return view('pages.admin.tanggapan.index', compact('tanggapans'));
}
...
{
    $pengaduan = Pengaduan::findOrFail($id);
    return view('pages.admin.tanggapan.create', compact('pengaduan'));
}
...
public function updateTanggapan(Request $request, $id)
{
    // Pastikan user login
    if (!auth()->check()) {
        return redirect()->back()->with('error', 'Anda harus login sebagai petugas untuk memberikan tanggapan.');
    }
    ...
    // Pastikan `petugas_id` selalu memiliki nilai
    $petugasId = auth()->user()->id ?? 1; // Ganti 1 dengan ID petugas default jika diperlukan
